...
class Space extends ContentContainerActiveRecord implements \humhub\modules\search\interfaces\Searchable
{
    ...
    public function scenarios()
    {
        ...
        $scenarios['edit'] = ['name', 'color', 'description', 'tags', 'join_policy', 'visibility', 'default_content_visibility', 'url', 'category'];
        $scenarios['create'] = ['name', 'color', 'description', 'join_policy', 'visibility', 'category'];
	...
	public static function sortByParameters(ActiveQuery $query, $param ) {
		switch ($param){
			...
			case 'ascdate' :
				...
				break;
			case 'like' :
				$subQuery = (new Query())->from('like')->select('object_id, COUNT(object_id) as count')->where(['object_model' => static::className()])->groupBy('object_id');
				$query->leftJoin(['like' => $subQuery],'like.object_id = '.static::tableName().'.id');
				$query->orderBy('count DESC');
				break;
			case 'favorite' :
				$subQuery = (new Query())->from('favorite')->select('object_id, COUNT(object_id) as count')->where(['object_model' => static::className()])->groupBy('object_id');
				$query->leftJoin(['favorite' => $subQuery],'favorite.object_id = '.static::tableName().'.id');
				$query->orderBy('count DESC');
			case 'members' :
				$subQuery = (new Query())->from('space_membership')->select('space_id, COUNT(space_id) as count_member')->where(['status' => 3])->groupBy('space_id');
				$query->leftJoin(['members' => $subQuery],'members.space_id = '.static::tableName().'.id');
				$query->orderBy('count_member DESC');
		}
		return $query;
	}
