...
class ContentActiveRecord extends ActiveRecord implements ContentOwner
{
	...
	public static function sortByParameters(ActiveQuery $query, $param ) {
        $contentQuery = (new Query())->select('object_id as object_id_content, created_at as created_at_content')->from('content')->where(['object_model' => static::className()]);
        $query->leftJoin(['content' => $contentQuery], 'object_id_content = '.static::tableName().'.id');
		switch ($param){
			case 'descdate' :
				$query->orderBy('created_at_content DESC');
				break;
			case 'ascdate' :
				$query->orderBy('created_at_content ASC');
				break;
			case 'like' :
